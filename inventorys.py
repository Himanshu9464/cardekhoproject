...
class carInventorySpage:
    ...
    def __init__(self,hp_window):
        ...
        self.window.title("Sold Inventory")
        # ---------- settings ------------------
        ...
        self.headlbl =Label(self.window,text="Car's Sold",background=mycolor2,font=("Cambria",35),
        ...
        self.L2 =Label(self.window,text="Old Owner ↓",background=mycolor1,font=myfont1)
        self.v1=StringVar()
        ...
        self.mytable2 = Treeview(self.window,columns=['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7','c8','c9','c10'], height=9)
        ...
        self.mytable2.heading("c4", text="Dob")
        self.mytable2.heading("c5", text="Address")
        self.mytable2.heading("c6", text="Aadhaar Id")
        self.mytable2.heading("c7",text="Car Brand")
        self.mytable2.heading("c8",text="Category")
        self.mytable2.heading("c9",text="Car Name")
        self.mytable2.heading("c10", text="Registration")
        ...
        y1+=v_diff
        self.L2.place(x=x1, y=y1+240)
        self.imglbl.place(x=x1 +900, y=y1 +240, height=300, width=300)
        self.mytable2.place(x=x1,y=y1+280)
    ...
    def searchData(self):
        try:
            qry = "select * from sellc where brand like %s"
            rowcount = self.curr.execute(qry,(self.v1.get()+"%"))
            data = self.curr.fetchall()
            self.mytable1.delete(*self.mytable1.get_children())

            qry1 = "select * from buyc where brand like %s and avail=0"
            rowcount1 = self.curr.execute(qry1, (self.v1.get() + "%"))
            ...
            if data1:  # data or
                for row1 in data1:
                    self.mytable2.insert("", END, values=(row1[0],row1[1],row1[2],row1[3],row1[4],row1[5],row1[6],row1[7],row1[8],row1[9]))
            else:
    ...
    def fetchData(self,pk_value=None):
        if pk_value==None:
            ...
            brand=pk_value
        try:
            qry = "select * from sellc where brand = %s"
...
if __name__ == '__main__':
    dummyHomepage=Tk()
    carInventorySpage(dummyHomepage)
