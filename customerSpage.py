...
class customerSpage:                    #BUY FROM CUSTOMER
    ...
    def __init__(self,hp_window):
        ...
        #----------------- background image ----------------------------
        from PIL import Image,ImageTk
        self.bkimg1 = Image.open("my_images//sellbg").resize((self.w,self.h))
        ...
        self.headlbl =Label(self.window,text="Sell To Customer",background=mycolor2,font=("Cambria",35),
        ...
        self.L13=Label(self.window,text="Price Ask",background=mycolor1,font=myfont1)
        ...
        self.mytable2 = Treeview(self.window,columns=['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8', 'c9', 'c10', 'c11'],height=12)

        self.mytable2.heading("c1", text="Cust. Name")
        self.mytable2.heading("c2", text="Phone")
        self.mytable2.heading("c3", text="Gender")
        self.mytable2.heading("c4", text="Aadhaar Id")
        self.mytable2.heading("c5", text="Car Brand")
        self.mytable2.heading("c6", text="Category")
        self.mytable2.heading("c7", text="Car Name")
        self.mytable2.heading("c8", text="Registration")
        self.mytable2.heading("c9", text="Manufacturing")
        self.mytable2.heading("c10", text="Fuel Type")
        self.mytable2.heading("c11", text="Price")

        self.mytable2['show'] = 'headings'
        self.mytable2.column("#1", width=85, anchor="center")
        self.mytable2.column("#2", width=85, anchor="center")
        self.mytable2.column("#3", width=85, anchor="center")
        self.mytable2.column("#4", width=85, anchor="center")
        self.mytable2.column("#5", width=85, anchor="center")
        self.mytable2.column("#6", width=85, anchor="center")
        self.mytable2.column("#7", width=85, anchor="center")
        self.mytable2.column("#8", width=85, anchor="center")
        self.mytable2.column("#9", width=85, anchor="center")
        self.mytable2.column("#10", width=85, anchor="center")
        self.mytable2.column("#11", width=85, anchor="center")

        self.mytable2.bind("<ButtonRelease>",lambda e: self.getpkValue1())
        ...
        self.b6 = Button(self.window,text="Search New",foreground=mycolor3,
                         background=mycolor2,font=myfont1,command=self.searchData1)
        ...
        self.b6.place(x=x1+h_diff+230,y=y1,height=30)
        y1+=v_diff
        ...
        self.imglbl.place(x=x1 +h_diff+h_diff+80, y=y1 , height=100, width=100)
        y1+=v_diff
        y1+=v_diff
        self.L6.place(x=x1,y=y1)
        self.t6.place(x=x1+h_diff,y=y1)

        self.mytable2.place(x=x1+h_diff+350,y=y1)
        y1+=v_diff
    ...
    def getImage(self):
        ...
        if self.filename!="":
            self.img1 = Image.open(self.filename).resize((100,100))
    ...
    def saveData(self):
        ...
        try:
            qry = "insert into sellc values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            rowcount = self.curr.execute(qry,(self.t1.get(),self.t2.get(),self.v1.get(),self.t4.get_date(),self.t5.get('1.0',END),
                                              ...
                                              self.t11.get_date(),self.v4.get(),self.t13.get(),self.actualname))
            self.conn.commit()
            qry1="update buyc set avail=0 where rn=%s"
            rowcount1 = self.curr.execute(qry1, (self.t10.get()))
            self.conn.commit()

            if rowcount==1 and rowcount1==1:
                messagebox.showinfo("Success ","Customer's Car Sold Successfully",parent=self.window)
                self.clearPage()
    ...
    def updateData(self):
        ...
        try:
            qry = "update sellc set name=%s, phone=%s , gender=%s , dob=%s, address=%s,aadhaar=%s, brand=%s, category=%s, cname=%s , " \
                  "md =%s ,ft =%s , price =%s , pic =%s where rn=%s "
            rowcount = self.curr.execute(qry,(self.t1.get(),self.t2.get(),self.v1.get(),self.t4.get_date(),self.t5.get('1.0',END),
    ...
    def deleteData(self):
        ...
        if (ans=="yes"):
            ...
            if self.oldname==self.default_img_name: # no image was given in past
                ...
                os.remove("customerimages//"+self.oldname)
            try:
                qry = "delete from sellc where rn=%s"
                rowcount = self.curr.execute(qry,(self.t10.get()))
                self.conn.commit()
                qry1 = "update buyc set avail=1 where rn=%s"
                rowcount1 = self.curr.execute(qry, (self.t10.get()))
                self.conn.commit()
                if rowcount==1 and rowcount1==1:
    ...
    def getpkValue1(self):
        rowID = self.mytable2.focus()
        data = self.mytable2.item(rowID)
        mycontent = data['values']
        pkValue1 = mycontent[7]
        self.fetchData1(pkValue1)
    ...
    def fetchData(self, pk_value=None):
        ...
        try:
            if Rn == pk_value:
                qry = "select * from buyc where rn = %s and avail=1"
                ...
                if data:
                    ...
                    self.img1 = Image.open("customerimages//" + self.oldname).resize((100, 100))
                    self.img2 = ImageTk.PhotoImage(self.img1)
                    self.imglbl.config(image=self.img2)
            ...
            messagebox.showerror("Query Error ", "Error in Query: \n" + str(e), parent=self.window)
    def fetchData1(self,pk_value1=None):
        if pk_value1==None :
            Rn=self.t10.get()
        else:
            Rn=pk_value1
        try:
            if Rn==pk_value1:
                qry = "select * from sellc where rn = %s"
                rowcount = self.curr.execute(qry,(Rn))
                data = self.curr.fetchone()
                self.clearPage()
                if data:
    ...
    def clearPage(self):
        ...
        self.img1 = Image.open("customerimages//"+self.default_img_name).resize((100, 100))
        self.img2 = ImageTk.PhotoImage(self.img1)
        self.imglbl.config(image=self.img2)
    ...
    def searchData(self):
        try:
            ...
            qry1 = "select * from sellc where name like %s"
            rowcount1 = self.curr.execute(qry1, (self.t1.get() + "%"))
            data1 = self.curr.fetchall()
            self.mytable2.delete(*self.mytable2.get_children())
            if data:  # or data1
                for row in data:
                    self.mytable1.insert("", END, values=(
                    row[0], row[1], row[2], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12]))
            elif data1:  # data or
                for row1 in data1:
                    self.mytable2.insert("", END, values=(
                    row1[0], row1[1], row1[2], row1[5], row1[6], row1[7], row1[8], row1[9], row1[10], row1[11],
                    row1[12]))
            else:
                messagebox.showinfo("Failure", "No Record Found", parent=self.window)
    ...
    def searchData1(self):
        try:
            qry = "select * from sellc where name like %s "
            rowcount = self.curr.execute(qry, (self.t1.get() + "%"))
            data = self.curr.fetchall()
            self.mytable2.delete(*self.mytable1.get_children())
            self.mytable2.delete(*self.mytable2.get_children())
            if data:
                for row in data:
                    self.mytable2.insert("", END, values=(row[0], row[1], row[2], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12]))
            else:
                messagebox.showinfo("Failure", "No Record Found", parent=self.window)
